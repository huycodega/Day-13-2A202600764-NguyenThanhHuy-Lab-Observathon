...
# Regex to catch injected instructions in order notes
...
def _sanitize(text: str) -> str:
    """Prefix order notes with a data-only marker to blunt prompt injection."""
...
def mitigate(call_next, question, config, context):
    ...
    # Cache lookup — skip for session-specific questions
    ...
    # Sanitize injected instructions in order notes
    clean_question = _sanitize(question)

    # Session reset to combat quality drift
    conf = dict(config)
